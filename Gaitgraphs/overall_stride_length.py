...
def calculate_average_stride_lengths(file_paths, sigma=2, window_size=20):
    results = []
...
def plot_average_stride_lengths(results_df, output_dir):
    ...
    # Create line plot for average stride lengths
    plt.figure(figsize=(10, 6))
    plt.plot(results_df['Day'], results_df['Avg_Left_Stride_Length'], marker='o', label='Left Stride Length', linestyle='-', color='blue')
    plt.plot(results_df['Day'], results_df['Avg_Right_Stride_Length'], marker='o', label='Right Stride Length', linestyle='-', color='orange')
    ...
    output_path = os.path.join(output_dir, 'average_stride_lengths_line_plot.png')
    plt.savefig(output_path)
    plt.close()
    print(f"Stride length comparison plot saved to {output_path}")
    print( results_df['Avg_Right_Stride_Length'])
    print( results_df['Avg_Left_Stride_Length'])
...
results = calculate_average_stride_lengths(file_list)
